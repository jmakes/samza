...
public class SingleStagePipelineFactory implements PipelineFactory {
  ...
  private PStream[] getInputs(Config config) {
    ...
    for (int i = 0; i < systemStreams.size(); i++) {
      streams[i] = new PStream(systemStreams.get(i), -1, null, PStream.Type.SOURCE, PStream.Visibility.PUBLIC);  // // TODO: 10/26/16 defaults for partition and key extractor not great
    }
    return streams;
  }
}
