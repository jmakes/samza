...
public class TestBasePipeline {
  ...
  public void setUp() {
    ...
    inputStream1 = createStream("InputStream1", PStream.Type.SOURCE, PStream.Visibility.PUBLIC);
    inputStream2 = createStream("InputStream2", PStream.Type.SOURCE, PStream.Visibility.PUBLIC);
    intermediateStream1 = createStream("IntermediateStream1", PStream.Type.INTERMEDIATE, PStream.Visibility.PRIVATE);
    intermediateStream2 = createStream("IntermediateStream2", PStream.Type.INTERMEDIATE, PStream.Visibility.PUBLIC);
    outputStream1 = createStream("OutputStream", PStream.Type.SINK, PStream.Visibility.PUBLIC);
  ...
  public void testBasicPipeline() {
    // BEFORE
//    Multimap<PStream, Processor> consumers = HashMultimap.create();
//    consumers.put(inputStream1, repartition);
//    consumers.put(inputStream2, repartition);
//    consumers.put(intermediateStream1, main);
//    consumers.put(intermediateStream2, main);
//
//    Multimap<PStream, Processor> producers = HashMultimap.create();
//    producers.put(intermediateStream1, repartition);
//    producers.put(intermediateStream2, repartition);
//    producers.put(outputStream1, main);
//
//    Pipeline pipeline = new BasePipeline(processors, streams, producers.asMap(), consumers.asMap());

    // AFTER
//    PipelineBuilder builder = new PipelineBuilder();
//    builder.addInputStreams(repartition, inputStream1, inputStream2);
//    builder.addIntermediateStreams(repartition, main, intermediateStream1, intermediateStream2);
//    builder.addOutputStreams(main, outputStream1);
//    Pipeline pipeline = builder.build();
  ...
  @Test
  public void testBasicPipelineByName() {
    PipelineBuilder builder = new PipelineBuilder();
    Pipeline pipeline = builder.addInputStreams("Repartitioner", "InputStream1", "InputStream2")
                                .addIntermediateStreams("Repartitioner", "MyProcessor", "IntermediateStream1", "IntermediateStream2")
                                .addOutputStreams("MyProcessor", "OutputStream")
                                .build();
  ...
  public void testValidationInvalidSourceStreamVisibility() {
    new PipelineBuilder().addInputStreams(repartition, createStream("InputStream3", PStream.Type.SOURCE, PStream.Visibility.PRIVATE)).build();
  }
  ...
  public void testValidationUnusedSourceStream() {
    new BasePipeline(Collections.singletonList(repartition), Collections.singletonList(inputStream1), Collections.emptyMap(), Collections.emptyMap());
  }
  ...
  public void testValidationIntermediateStreamVisibilityPrivate() {
    PStream privateStream = createStream("IntermediateStream3", PStream.Type.INTERMEDIATE, PStream.Visibility.PRIVATE);
  ...
  public void testValidationInvalidSinkStreamVisibility() {
    new PipelineBuilder().addInputStreams(repartition, inputStream1) // Avoid error for no source streams
        .addOutputStreams(repartition, createStream("OutputStream2", PStream.Type.SINK, PStream.Visibility.PRIVATE)).build();
  }
...
//  }
  ...
  private PStream createStream(String name, PStream.Type type, PStream.Visibility visibility) {
    return new PStream(new SystemStream("kafka", name), 4, null, type, visibility);
  }
}
